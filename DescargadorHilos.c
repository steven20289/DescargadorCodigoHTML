...
typedef struct _Ruta
{
	char ruta[_LARGO];
	char link[_LARGO];
	char direccion_descarga[_LARGO];
} Ruta;
 ...
 */
void* descargador_archivos(void* struct_ruta_temp)
{
	
		Ruta* struct_ruta;
		struct_ruta = (Ruta*)struct_ruta_temp;
		char archivo[_LARGO];
		strcpy(archivo,struct_ruta->direccion_descarga);
		strcat(archivo,struct_ruta->ruta);
		
		char *url;
		url = struct_ruta->link;		
 ...
 */
void extraer_archivo(char *archivo)
{
		char direccion_descarga_t[_LARGO];
		strcpy(direccion_descarga_t,direccion_descarga);
		
		strcat(direccion_descarga_t, archivo);
		strcpy(archivo,direccion_descarga_t);
		...
		(!seek_string(link,".bmp"))&&(!seek_string(link,".php"))&&(!seek_string(link,".asp")))
		{
			...
			{							
				...
				{		
					...
					if ((nuevo_elemento = (Ruta *) malloc (sizeof (Ruta))) == NULL)
					{
						return;
					}
					
					strcpy(nuevo_elemento->ruta,nombre);
					strcpy(nuevo_elemento->link,link);
					strcpy(nuevo_elemento->direccion_descarga, direccion_descarga);
					
					err = pthread_create(&(tid[_thread]), NULL, descargador_archivos,(void *)nuevo_elemento);
					if (err != 0)
						printf("\ncan't create thread :[%s]", strerror(err));
					
					pthread_join(tid[_thread], NULL);
